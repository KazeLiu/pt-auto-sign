...
- **前台激活页面** 打开后，当前页面签到就会在前台打开，会打断电脑正常使用，用于cloudflare验证，如果你感觉放在后台盾也不会拦截，那就不需要打开
- **是否启用** 在签到主页中是否显示该站点
- **我的站点需要定制签到流程** 我没有的站那肯定是没有对应的签到流程了，毕竟不知道签到长啥样

## 站点类型和签到地址的详细解释：
举个例子，有个站是Rousi,它的签到不是访问`/attendance`，所以不是通用的`nexusPHP`。进入首页后发现有个签到按钮，那么签到地址填写首页的地址：`https://rousi.pro`，然后在站点类型选择`ROUSI`，添加完成后在主页点击"手动签到"，看能不能成功签到。

其他的站也是这样的，把签到按钮所在的页面的地址填写到签到地址中，然后站点类型选择对应的站点名称。
## 其他功能
- **签到主页** 这是签到主页，点击签到按钮就可以签到
  - 再次签到：点一下执行单个的签到流程
  - 网址：点一下跳转到站点
- **IYUU 推送** 去IYUU网站绑定后填入就可以推送消息了
- **批量打开后台签到页** 一键签到同时打开全部的不在前台激活的站点，可以省点时间
